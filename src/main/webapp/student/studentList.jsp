...
$(function() {
	...
	$("#linkStudentDel").click(function(event) {
		if (!confirm("真的确定要删除!")) {
	...
	$(".glyphicon-trash").click(function() {
		...
		if (vals.length<=0) {
			...
			if (!confirm("真的确定要删除？？？？")) {
				...
				//删除操作
				$.post("${pageContext.request.contextPath}/admin/studentsDel.action",{
					ids:vals
				},function(data){
					window.location.href="${pageContext.request.contextPath}/admin/studentList.action";
				});
			}
		}
	});
...
<body>
	<form class="form-inline well-sm" id="search" action="${pageContext.request.contextPath}/admin/studentList.action" method="post">
	...
	<table class="table table-hover table-bordered">
		...
		<tbody>
			...
			<s:iterator value="#request.pageBean.recordList" var="student" status="rowstatus">
				<s:if test="#rowstatus.odd == true">
					<tr class="success">
						<td><input type="checkbox" value='<s:property value="#student.id"/>' name="uid"></td>
						...
						<td><s:property value="#student.memo" /></td>
						<td><s:url var="studentDelLink" action="studentDel" namespace="/admin">
								<s:param name="id" value="#student.id"></s:param>
							</s:url> <s:url var="studentViewLink" action="studentView" namespace="/admin">
								<s:param name="id" value="#student.id"></s:param>
							</s:url> <s:url var="studentEditLink" action="toStudentEdit" namespace="/admin">
								<s:param name="id" value="#student.id"></s:param>
							</s:url> &nbsp;&nbsp; <a title="编辑" class="glyphicon glyphicon-edit" href="${studentEditLink}" id='<s:property value="#student.id"/>'> </a> &nbsp;&nbsp; <a title="查看" class="glyphicon glyphicon-eye-open" href="${studentViewLink}" target="mainContent" id='<s:property value="#student.id"/>'></a> &nbsp;&nbsp; <a id="linkStudentDel" title="删除" href="${studentDelLink}" target="mainContent"> <i
			...
			<td><a title="删除选中的行" class="glyphicon glyphicon-trash" href="javascript:void(0)" id=''></a>&nbsp;&nbsp;&nbsp;&nbsp; <s:url var="toStudentAdd" action="toStudentAdd" namespace="/admin"></s:url> <a title="增加一条数据" class="glyphicon glyphicon-plus" href="${toStudentAdd}" id=''></a></td>
